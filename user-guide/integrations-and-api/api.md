...
All the API calls support authentication with an API key. DataReporter has two types
of API keys:
...
We provide a light wrapper around the DataReporter API called `redash-toolbelt`. It's a work-in-progress. The source code is hosted on [Github](https://github.com/getredash/redash-toolbelt). The `examples` folder in that repo includes useful demos, such as:
...
> **Danger:** Below is an incomplete list of DataReporter's API endpoints as of V9. These may change in future versions of DataReporter.

Each endpoint is appended to your DataReporter base URL. For example:
...
> **Info:** Here's an example JSON object including different parameter types:
> 
> ```
> {
>   "parameters": {
