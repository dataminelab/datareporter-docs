...
While there are many ways to define the stages of a Cohort analysis, DataReporter's supports Cohorts visualizations with daily, weekly, or monthly stages. Also, DataReporter's cohort charts compare a cohort's measurements in a given period against that group's initial population size.

## Data Format

DataReporter expects your input samples to take the following format:
...
- **Total Cohort Size** is the denominator that DataReporter will use to calculate the percentage of a cohort's target satisfaction for a given period. Continuing the example above, if seventy-two users signed up in January then the Total Cohort Size would be 72. When the visualization is rendered, DataReporter would display the value as 41.67% (30 ÷ 72).

## Cohort Date Notes

Even if you define your cohorts by month or week, DataReporter expects the values in your **Cohort Date** column to be a full date value. If you are grouping by month, `2018-01-18` should be shortened to `2018-01-01` or any other full date in January, **not** `2018-01`.

Due to a bug in the underlying rendering engine, DataReporter's cohort visualizer converts all date and time values to GMT before rendering. To avoid rendering issues, you should adjust the datetimes returned from your database by your local UTC offset.
