...
DataReporter's pivot table visualization can aggregate records from a query result into a new tabular display. It's similar to `PIVOT` or `GROUP BY` statements in SQL. But the visualization is configured with drag-and-drop fields instead of SQL code.
...
![Example Query for Pivot Table](/docs/images/gitbook/pivot-table-query.png)
...
![](/docs/images/gitbook/pivot-table-configuration-examples.png)

> **Warning:** Pivot table performance can degrade if your query result is too big. The exact size threshold will depend on the computer and browser from which you access DataReporter. But in general, performance is best below 50,000 _fields_. That could mean 10,000 records with 5 fields each. Or 1,000 records with 50 fields each.
