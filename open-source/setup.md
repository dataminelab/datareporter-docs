...
> **Info:** We have not updated our official images for V10 yet. However, you can deploy a V8 instance and upgrade it following [these instructions](https://github.com/getredash/redash/releases/tag/v10.0.0).
> 
...
First, you need to add the DataReporter image to your account:
...
Once you created the instance with either the image or the script, you should have a running DataReporter instance with everything you need to get started. DataReporter should be available using the server IP or DNS name you assigned to it. You can point your browser to this address.

Before you can continue, it will ask you to create your admin account. Once this is done, you can start using DataReporter.

![Initial Setup Screen](/docs/images/redash_initial_setup.png)

> **Warning:** Make sure to complete the web based setup before using the CLI or proceeding with the rest of the setup.

To make your setup more complete, there are a few more steps that you need to manually do:

### Mail Configuration

For the system to be able to send emails (user invites, password resets, when alerts trigger and more), you need to configure DataReporter with the mail server you use. Mail server configuration is done using Envrionment Variables. If you’re using one of our images, you can do this by editing the `/opt/redash/env` file.
...
Also you need to set the value of `REDASH_HOST`, which is the base address of your DataReporter instance (the DNS name or IP) with the protocol, so for example: `https://demo.redash.io`.
...
Once updated, restart the web server (`docker-compose up -d server`). Once enabled, DataReporter will use Google OAuth to authenticate _existing_ user accounts. To enable automatic user creation who belong to a specific domain name, you can add this domain (or more) in the setting page:

![](/docs/images/redash_google_oauth_domain.png)
...
> **Warning:** After your installation is complete, you can do the healthcheck by calling `/ping` API endpoint.
