...
> **Important:** **Starting from the DataReporter V8 release we're no longer building the tarball version of a release – only Docker images.** This of course doesn’t prevent you from deploying DataReporter anyway you want, but the `bin/upgrade` script won't work with the new releases.
...
> **Info:** If you're upgrading to a beta version, you need to specify the channel: `sudo bin/upgrade --channel beta`.
...
2. Apply migrations (if needed). To apply migrations you need to invoke the image with `manage db upgrade` as the command. The way to do this depends on the way you deploy DataReporter.
...
> **Note:** You can upgrade to v1.0.0 and later only from v0.12.0, so if you have an older version, run: `sudo upgrade --channel legacy` first and then `sudo upgrade`.
