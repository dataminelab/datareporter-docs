...
DataReporter encrypts secret information with two keys: the Cookie Secret and the Application Secret.
...
If you attempt to start DataReporter without it, the webserver will not initialise and you will find the following message in your logs:
...
It is required to start the application. However, if you do not set one explicitly, DataReporter will use the cookie secret instead. This is helpful for development. But for maximum security, we recommend you set a unique value for both variables.
...
A strong secret key will be random and impossible to guess. Do not reuse secret keys across instances of DataReporter or commit them to version control.
...
Under normal circumstances, you configure your secret keys while deploying DataReporter and will not touch them again. However, if a key is compromised (by committing it to a version control system, for example) you should change it immediately.
...
If you did not already explicitly set `REDASH_SECRET_KEY`, then you should use the instructions in the next section to reencrypt your secret fields before modifying `REDASH_COOKIE_SECRET`. Otherwise your existing data sources will be inaccessible to DataReporter.

### Changing the Application Secret

Because DataReporter encrypts secret fields at rest in its internal database, if you change `REDASH_SECRET_KEY` you must also reencrypt these fields. Otherwise you will not be able to execute queries, modify your data sources, or even access the data source settings screen.

> **Danger:** Do not delete your `REDASH_SECRET_KEY` until you have finished re-encrypting secret fields. Without it your data sources will become unusable.

#### Re-encrypting Secret Fields

The DataReporter CLI includes the `database reencrypt` command to conveniently re-encrypt your secret fields stored by DataReporter. It accepts two positional arguments: the old secret and the new secret. When you run the command, the secret fields in DataReporter's internal database are decrypted using the old secret and encrypted again with the new secret. This includes data source definitions and alert destinations.

If you deployed DataReporter using docker-compose (from one of our cloud images, for instance) you can invoke this CLI on your docker host with this command:
