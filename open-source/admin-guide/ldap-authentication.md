...
Starting from DataReporter v3, there is direct support for LDAP/AD authentication. To
...
- `REDASH_LDAP_LOGIN_ENABLED`: When `true` LDAP login will be enabled at
  ...
  `false`, users will be prevented from logging in to DataReporter using DataReporter's
...
If you're using Docker Compose to run DataReporter, you can replace
