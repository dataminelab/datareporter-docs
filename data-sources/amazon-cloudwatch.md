...
DataReporter connects to CloudWatch using the `boto3` client. According to their [documentation](https://boto3.amazonaws.com/v1/documentation/api/latest/guide/quickstart.html) before you can query with DataReporter...
...
![CloudWatch Schema Example](/docs/images/gitbook/cloudwatch-schema.png)

Your query text must written in [YAML syntax](https://yaml.org/start.html) (not SQL). It should be an array of `MetricDataQuery` objects under a key called `MetricsDataQueries`. When you execute the query DataReporter converts this array to one or more `get_metric_data()` calls.
...
> **Info:** To query your CloudWatch data with SQL, first pull data using the YAML syntax. Then use QRDS to filter and sort the results
...
Your query _must_ include either a `logGroupName` or `logGroupNames`. When you execute the query DataReporter converts this object into a `start_query()` call. Here is an example query:
