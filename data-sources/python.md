...
- **Modules to import prior to running the script** lets you define which modules that were installed by `pip` on the host server may be import in DataReporter queries.
- **AdditionalModulesPaths** is a comma-separated list of absolute paths _on the DataReporter server_ to Python modules that should be available when querying from DataReporter. This is useful for private modules that are not available from `pip`.
- **AdditionalBuiltins** DataReporter automatically allows twenty-five of Python's built-in functions that are considered safe. You can specify others here.
...
DataReporter builds the result table by inspecting the final state of your script execution for a variable named `result`.
...
If you execute the above snippet in DataReporter it will return this table:
